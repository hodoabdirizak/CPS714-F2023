...
export const ProfileOrganizer = () => {
  ...
  const getOrganizerAccount = async () => {
    let response = await fetch('/api/organizer/getorganizeraccount', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
    ...
    const items = data.split("|");
      setUserAccount(prevState => ({
        ...prevState,
        Description: `${items[0] === 'null' ? '' : items[0]}`, 
        Website: `${items[1] === 'null' ? '' : items[1]}`
      }));
    return;
  }

  const updateUserAccount = async () => {
    let response = await fetch('/api/organizer/updateuseraccount', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
  ...
  const deleteUserAccount = async () => {
    console.log('called');
    let response = await fetch('/api/organizer/deleteaccountorganizer', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
  ...
  const goBack = (e) => {
    history.push('/', { isLoggedIn: 'true', username: email, accountType: "Organizer" });
  };
        ...
        <form onSubmit={handleProfileUpdate}>
          ...
          <div className="form-group-item">
            <input
              ...
              value={newUserAccount.Description}
              onChange={setInput}
              required
              className="input-style-5"
            />
