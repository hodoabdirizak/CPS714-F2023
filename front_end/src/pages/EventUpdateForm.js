...
export const EventUpdateForm = () => {
  ...
  const eventID = location.state?.eventID || 35;
...
const handleSubmit = (e) => {
  ...
  if(e.nativeEvent.submitter.value === "Update"){
    ...
    if (window.confirm("Are you sure you want to delete your event") == true) {
      deleteEvent();
...
const updateEvent = async () => {
  await fetch('/api/event/updateEvent', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json'
...
const deleteEvent = async () => {
  await fetch('/api/event/deleteEvent', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json'
      ...
      <div>
        <input
          ...
          <button type="submit" value ="Update" name="btn">Update</button>
            <div></div>
            <button type="submit" value ="Delete" name="btn">Delete Event</button>
        </form>
      </div>
    </div>
  </div>
